...
const HealthcareProviderManagement = () => {
  ...
  const fetchAllHealthcareProviders = () => {
    HealthcareProviderService.getAllHealthcareProviders()
      .then((response) => {
        ...
        setTotalProviders(response.data.length); // Updating total providers count
      })
      .catch((error) => {
  ...
  const handleAdd = () => {
    setShowForm(true);
    setProviderToUpdate(null); // Resetting providerToUpdate for add operation
  };
  ...
  // Filtering healthcare providers based on search term
